...
# Run
docker run -d -v "$PWD"/init-db.sql:/docker-entrypoint-initdb.d/init-db.sql -v laborator1-db-persistent-volume:/var/lib/postgresql/data --network=laborator1-db-network -e POSTGRES_USER=admin -e POSTGRES_PASSWORD=admin -e POSTGRES_DB=books --name laborator1-db postgres:16
